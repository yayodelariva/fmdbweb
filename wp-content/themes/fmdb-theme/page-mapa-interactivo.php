<?php
/**
 * Template: Mapa Interactivo
 * Automatically used by WordPress for the page with slug "mapa-interactivo"
 */
get_header();
...
// Fetch all published asociaciones
$all_asociaciones = get_posts( [
    'post_type'      => 'fmdb_asociacion',
    'posts_per_page' => -1,
    'post_status'    => 'publish',
    'orderby'        => 'title',
    'order'          => 'ASC',
...
// Collect unique states from asociaciones for the asociaciones filter
$asociacion_states = [];
foreach ( $all_asociaciones as $asoc ) {
    $s = get_field( 'asociacion_state', $asoc->ID );
    if ( $s ) $asociacion_states[ $s ] = true;
}
$asociacion_states = array_keys( $asociacion_states );
sort( $asociacion_states );
?>

<main id="fmdb-equipos" class="fmdb-equipos" data-view="todos" data-mode="mapa">

    <div class="fmdb-equipos__header">
        <h1>Asociaciones, Ligas y Equipos de México</h1>
    ...
    <div class="fmdb-equipos__controls">
        <div class="fmdb-equipos__toggle" role="tablist" aria-label="Vista del mapa">
            <button class="fmdb-equipos__toggle-btn" data-view="asociaciones" type="button" role="tab">
                <span class="fmdb-toggle-btn__prefix">Mostrar solo </span>Asociaciones <span class="fmdb-tab-count"><?php echo count( $all_asociaciones ); ?></span>
            </button>
            <button class="fmdb-equipos__toggle-btn" data-view="ligas" type="button" role="tab">
                <span class="fmdb-toggle-btn__prefix">Mostrar solo </span>Ligas <span class="fmdb-tab-count"><?php echo count( $all_leagues ); ?></span>
            </button>
            <button class="fmdb-equipos__toggle-btn" data-view="equipos" type="button" role="tab">
                <span class="fmdb-toggle-btn__prefix">Mostrar solo </span>Equipos <span class="fmdb-tab-count"><?php echo count( $all_teams ); ?></span>
            </button>
            <button class="fmdb-equipos__toggle-btn active" data-view="todos" type="button" role="tab" aria-selected="true">
                <span class="fmdb-toggle-btn__prefix">Mostrar </span>Todos
            </button>
        </div>
    ...
    <div class="fmdb-equipos__map">
        ...
        <div class="fmdb-map-legend">
            <span><i style="background:#D3D1C7"></i><span id="fmdb-map-legend-empty">Sin equipos</span></span>
    ...
    <!-- Shared state filter (drives equipos + ligas + asociaciones panels) -->
    <div class="fmdb-equipos__filters fmdb-equipos__filters--shared">
        <div class="fmdb-filter-group">
            <label for="filter-state">Estado</label>
            <select id="filter-state">
                <option value="">Todos los estados</option>
                <?php foreach ( fmdb_mexican_states() as $s ) : ?>
    ...
    <!-- ============ ASOCIACIONES PANEL ============ -->
    <div class="fmdb-equipos__panel" data-panel="asociaciones">
        <h2 class="fmdb-equipos__panel-title">Asociaciones estatales</h2>

        <div class="fmdb-equipos__filters">
            <div class="fmdb-filter-group">
                <label for="filter-state-asociaciones">Estado</label>
                <select id="filter-state-asociaciones">
                    <option value="">Todos los estados</option>
                    <?php foreach ( $asociacion_states as $s ) : ?>
                        <option value="<?php echo esc_attr( $s ); ?>"><?php echo esc_html( $s ); ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <button type="button" id="fmdb-clear-filters-asociaciones" class="fmdb-btn-ghost">Limpiar filtros</button>
        </div>

        <?php if ( empty( $all_asociaciones ) ) : ?>
            <p class="fmdb-empty">No hay asociaciones registradas aún.</p>
        <?php else : ?>
            <div class="fmdb-asociacion-grid">
                <?php foreach ( $all_asociaciones as $asoc ) :
                    $a_state    = get_field( 'asociacion_state', $asoc->ID );
                    $a_desc     = get_field( 'asociacion_description', $asoc->ID );
                    $a_thumb    = get_the_post_thumbnail_url( $asoc->ID, 'thumbnail' );
                    $a_words    = array_filter( explode( ' ', $asoc->post_title ) );
                    $a_initials = substr( implode( '', array_map( fn( $w ) => strtoupper( $w[0] ), $a_words ) ), 0, 3 );
                ?>
                    <a href="<?php echo esc_url( get_permalink( $asoc->ID ) ); ?>"
                       class="fmdb-asociacion-card"
                       data-state="<?php echo esc_attr( $a_state ?: '' ); ?>">
                        <div class="fmdb-asociacion-card__crest">
                            <?php if ( $a_thumb ) : ?>
                                <img src="<?php echo esc_url( $a_thumb ); ?>" alt="<?php echo esc_attr( $asoc->post_title ); ?>">
                            <?php else : ?>
                                <span><?php echo esc_html( $a_initials ); ?></span>
                            <?php endif; ?>
                        </div>
                        <div class="fmdb-asociacion-card__body">
                            <strong><?php echo esc_html( $asoc->post_title ); ?></strong>
                            <?php if ( $a_state ) : ?>
                                <small><?php echo esc_html( $a_state ); ?></small>
                            <?php endif; ?>
                            <?php if ( $a_desc ) : ?>
                                <p><?php echo esc_html( wp_trim_words( $a_desc, 22 ) ); ?></p>
                            <?php endif; ?>
                        </div>
                    </a>
                <?php endforeach; ?>
            </div>
            <div id="fmdb-asociaciones-no-results" class="fmdb-empty" style="display:none;">
                No se encontraron asociaciones con los filtros seleccionados.
            </div>
        <?php endif; ?>
    ...
    <div class="fmdb-equipos__panel" data-panel="ligas">
        ...
        <div class="fmdb-equipos__filters">
            ...
            <button type="button" id="fmdb-clear-filters-ligas" class="fmdb-btn-ghost">Limpiar filtros</button>
        </div>
